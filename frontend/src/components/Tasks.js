...
const Tasks = () => {
  ...
  useEffect(() => {
    axios
      .get("/api/users/usersWithTasks", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("jwt_token")}`,
        },
      })
      .then((res) => {
        setUsersWithTasks(res.data);
      })
      .catch((err) => {
        console.log(err);
      });
      ...
      <div className="calendar">
        <div className="header row flex-middle">
          ...
          <div className="col col-center">
            <span className="api-text">My tasks</span>
          </div>
          ...
          .map((task) => (
            <div key={task.id} className="header tasks row flex-middle">
              <div className="col col-start">
                ...
                />
                <span
                  className={`api-text text-wrap ${task.completed ? "completed" : ""
                    }`}
                >
                  {task.title}
                </span>
          ...
          <div className="calendar">
            <div className="header row flex-middle">
              <div className="col col-center text-center">
                <span className="api-text">User '{user.email}' tasks</span>
              </div>
            </div>
